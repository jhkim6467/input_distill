...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    #    model_num = opt.output.split(".")[1]

    #if opt.gpu > -1:
    torch.cuda.set_device(1)

    opt.models = ['distill_files/model0.pt']
    opt.data_type = 'text'
...
class Translator(object):
    ...
    def translate(self,
        ...
        import pickle
        with open('distill_files/input_emb_{}.pkl'.format(self.model_num), 'wb') as f:
            pickle.dump(enc_out, f)
    ...
    def _translate_batch(self, text):
        ...
        temp_src = []
        for index in text:
            temp_src.append([vocab.stoi[index]])
        src.append(temp_src)
        for i in range(batch_size - 1):
            src.append([[1] for i in np.ones(len(text))])
        src = np.transpose(src)

        src_lengths = []
        for index in range(batch_size):
            if index == 0:
                src_lengths.append(len(text))
            else:
                src_lengths.append(0)

        src_lengths = torch.tensor(src_lengths)
        src = torch.LongTensor(src)
        src = src.view(len(text), batch_size, -1)

        enc_states, memory_bank = self.model.encoder(src, src_lengths)

        emb = torch.cat((enc_states[0][0], enc_states[0][1]), 1)
